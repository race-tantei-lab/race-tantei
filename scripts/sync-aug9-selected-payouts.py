...
def parse_text_payouts(page: str):
    ...
    if endings:
        segment = segment[: min(endings)]
    type_matches = list(re.finditer(r"(?:^|\s)(単勝|複勝|枠連|ワイド|馬連|馬単|3連複|3連単)(?=\s)", segment))
    payouts = []
    for idx, type_match in enumerate(type_matches):
        bet_type = type_match.group(1)
        start = type_match.end()
        end = type_matches[idx + 1].start() if idx + 1 < len(type_matches) else len(segment)
        ...
        pattern = re.compile(rf"({combo_pattern})\s+([0-9,]+)\s*円(?:\s+(\d+)\s*番人気)?")
        for match in pattern.finditer(chunk):
            combo = normalize_combination(match.group(1))
            if not combo:
                continue
            payouts.append({"betType": bet_type,"combination": combo,"payoutYen": int(match.group(2).replace(",", "")),"popularity": int(match.group(3)) if match.group(3) else None})
    return payouts


def parse_payouts(page: str):
    payouts = parse_table_payouts(page)
    if not payouts:
        payouts = parse_text_payouts(page)
    unique = {}
    for row in payouts:
...
def main():
    ...
    for race in finished:
        ...
        try:
            ...
            if not {"単勝","馬連","馬単","ワイド","3連複","3連単"}.issubset(types):
                raise RuntimeError(f"PAYOUT_TYPES_INCOMPLETE:{sorted(types)}")
            for p in payouts:
                ...
                inserted += 1
            synced.append({"raceId": race_id,"payoutRows": len(payouts),"betTypes": sorted(types)})
