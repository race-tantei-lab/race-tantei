...
const split = (values, size) => Array.from(
...
function ticketScore(ticket) {
  return Math.max(0.01, ticket.expectedValuePct / 100)
    * Math.sqrt(Math.max(0.000001, ticket.hitProbability));
}
...
function allocate(tickets, target, limit) {
  ...
  return rows.map((row, index) => ({
    ...row,
    stakeYen: (base[index] + add[index]) * 100
  }));
}
...
function portfolio(race, course, policy, target) {
  ...
  if (!rows.length && original.length) {
    rows = [[...original].sort((a, b) => ticketScore(b) - ticketScore(a))[0]];
  }
...
function selected(records, count) {
  ...
  return [...groups.values()].flatMap((rows) => rows
    .sort((a, b) => b.score - a.score || a.raceNo - b.raceNo)
...
function metrics(records, course, policy, target) {
  ...
  const minMonth = months.size
    ? Math.min(...[...months.values()].map((row) => row.stake ? row.returned / row.stake * 100 : 0))
    : 0;
  ...
  return {
    ...
    roi,
    hit,
    ticketHit: tickets ? ticketHits / tickets * 100 : 0,
    minMonth,
    ...
    for (const minEv of [0, 110, 120, 130, 150, 180]) {
      for (const minHit of [0, 0.04, 0.08, 0.12, 0.18]) {
        for (const maxOdds of [12, 30, 80, 2500]) {
          for (const maxTickets of [1, 2, 3, 5, 8]) {
            rows.push({ profile, minEv, minHit, maxOdds, maxTickets });
          }
        }
      }
    }
...
function bestPolicy(records, course, target, baseline) {
  ...
  const nonWorse = candidates.filter((row) => row.result.hit >= baseline.hit);
  const eligible = improved.length ? improved : nonWorse;
...
async function main() {
  ...
  const records = races.flatMap((race) => {
    ...
    const payoutMap = new Map((payouts.get(race.raceId) ?? []).map((row) => [
    ...
    return [{
      ...
      score: betting.learnedPredictionRaceScore(predictions),
      tickets
    }];
  });
  ...
  const baseline = Object.fromEntries(courses.map((course) => [
    course,
    metrics(baselineRows, course, current, budget.COURSE_TARGET_STAKES[course])
  ]));

  const candidates = [];
  const skippedRaceCounts = [];

  for (const count of [5, 6, 7, 8, 9, 10, 12]) {
    console.log(`Analyzing ${count} races per venue/day...`);
    const rows = selected(validation, count);
    const policyEntries = courses.map((course) => [
      course,
      bestPolicy(rows, course, budget.COURSE_TARGET_STAKES[course], baseline[course])
    ]);

    const failedCourses = policyEntries
      .filter(([, policy]) => policy === null)
      .map(([course]) => course);
    if (failedCourses.length) {
      skippedRaceCounts.push({ count, failedCourses });
      console.log(`Skipping ${count}R: baseline hit rate could not be preserved for ${failedCourses.join(", ")}`);
      continue;
    }
    ...
    candidates.push({
      count,
      policies,
      score: minRoi * 0.6 + minMonth * 0.3 + minHit * 0.1,
      minRoi,
      minHit,
      minMonth
    });
  ...
  if (!winner) {
    throw new Error("NO_RACE_COUNT_PRESERVES_BASELINE_HIT_RATE");
  }

  const holdoutRows = selected(holdout, winner.count);
  const final = Object.fromEntries(courses.map((course) => [
    course,
    metrics(
      holdoutRows,
      course,
      winner.policies[course].policy,
  ...
  const report = {
    ...
    eligibleRaceCounts: candidates.map((candidate) => candidate.count),
    skippedRaceCounts,
    baseline: Object.fromEntries(courses.map((course) => [course, {
      roi: round(baseline[course].roi),
      hit: round(baseline[course].hit)
    }])),
    ...
    holdout: Object.fromEntries(courses.map((course) => [course, {
      roi: round(final[course].roi),
      hit: round(final[course].hit),
      profit: final[course].profit,
      pass200: final[course].roi >= 200
    }]))
  };
  ...
  for (const course of courses) {
    console.log(`${course}: ROI ${report.baseline[course].roi}% / hit ${report.baseline[course].hit}%`);
  }
  console.log(`\n=== Validation winner: ${winner.count}R per venue/day (minimum 5 preserved) ===`);
  for (const course of courses) {
    console.log(`${course}: ROI ${report.validation[course].roi}% / hit ${report.validation[course].hit}% / ${JSON.stringify(report.validation[course].policy)}`);
  }
  console.log("\n=== Untouched holdout ===");
  for (const course of courses) {
    console.log(`${course}: ROI ${report.holdout[course].roi}% / hit ${report.holdout[course].hit}% / 200% ${report.holdout[course].pass200 ? "PASS" : "FAIL"}`);
  }
