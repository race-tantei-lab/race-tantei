    ...
    for case_index in range(96):
        ...
        for bet_type in BET_ORDER:
            for pos in positions(bet_type, n):
                ...
                odd = (low + high) / 2.0
                odds[(f"case-{case_index}", bet_type, combination)] = odd
        ...
        chosen = generator.choose_two(core, f"case-{case_index}", runners, weights, odds)
        cases.append({"id": f"case-{case_index}", "horseNos": horse_nos, "weights": weights, "odds": rows, "expected": chosen})
