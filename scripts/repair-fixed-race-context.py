...
def parse_context(page, row):
    ...
    patterns = (
        r"コース\s*[：:]\s*([0-9,]+)\s*メートル\s*[（(]([^）)]{1,100})[）)]",
        r"([芝ダート障害]+)\s*([0-9,]{3,5})\s*[mｍメートル].{0,30}?(右|左|直線)?",
        r"([0-9,]{3,5})\s*[mｍメートル].{0,30}?(芝|ダート|障害).{0,20}?(右|左|直線)?",
    )
    ...
    if m:
        ...
        if m:
            ...
            course_inside = compact(m.group(0))
        else:
    ...
    weather = ""
    m = re.search(r"天候\s*[：:]?\s*([晴曇雨雪小雨小雪]+)", text)
    if m:
        weather = m.group(1)

    track = ""
    m = re.search(r"(?:芝|ダート)\s*[：:]?\s*(良|稍重|重|不良)", text)
    if m:
        track = m.group(1)
...
for row in rows:
    ...
    if surface and distance > 0:
        results.append({
            ...
            "raceName": compact(row.get("raceName")),
            "conditions": compact(row.get("conditions")),
            "weather": compact(row.get("weather")),
            "trackCondition": compact(row.get("trackCondition")),
            "courseText": "",
            "sourceUrl": row.get("resultUrl"),
            "parsed": True,
            "source": "existing_d1",
        })
    else:
...
with gzip.open(OUT, "wt", encoding="utf-8", compresslevel=6) as f:
    json.dump({"schema": 2, "fixedArchiveOnly": True, "rows": results}, f, ensure_ascii=False)
META.write_text(
    json.dumps(
        {
            "races": EXPECTED_RACES,
            "alreadyValid": EXPECTED_RACES - len(missing),
            "fetched": len(missing),
            "parsed": len(parsed),
            "failed": len(failed),
            ...
            "workers": WORKERS,
            "failedExamples": [
                {"raceId": r.get("raceId"), "date": r.get("raceDate"), "url": r.get("sourceUrl"), "error": r.get("error")}
                for r in failed[:50]
            ],
        },
        ensure_ascii=False,
...
print(json.dumps({"parsed": len(parsed), "failed": len(failed), "coveragePct": coverage * 100.0, "surfaceCounts": surface_counts}, ensure_ascii=False), flush=True)
if coverage < 0.98:
