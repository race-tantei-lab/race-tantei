...
def main():
    ...
    if train['raceId'].nunique()<1000 or ev['raceId'].nunique()<1000 and year not in (2016,2026):raise RuntimeError(f'FOLD_SIZE_INVALID:{year}:{train["raceId"].nunique()}:{ev["raceId"].nunique()}')
    ...
    with out.open('w',encoding='utf-8') as f:
        for i,r in ev.iterrows():
            f.write(json.dumps({'raceId':str(r['raceId']),'raceDate':str(r['raceDate']),'venue':str(r['venue']),'raceNo':int(r['raceNo']),'horseNo':int(r['horseNoRaw']),'finishPosition':int(r['finishPosition']),'marketPopularity':float(r['marketPopularity']) if not pd.isna(r['marketPopularity']) else None,'abilityScore':float(outputs['ability'][i]),'marketScore':float(outputs['market'][i]),'targetRaceResultUsedForPrediction':False},ensure_ascii=False,separators=(',',':'))+'\n')
    meta={'purpose':'research_only_all_races_year_walkforward_runner_ranker','year':year,'trainEndExclusive':train_end,'evaluationStart':eval_start,'evaluationEnd':eval_end,'trainingRaces':int(train['raceId'].nunique()),'predictionRaces':int(ev['raceId'].nunique()),'predictionRunners':len(ev),'metrics':met,'futureRowsUsedForTraining':False,'sameDayRowsUsedForTraining':False,'syntheticDataUsed':False,'productionDatabaseWritten':False,'productionModelChanged':False}
