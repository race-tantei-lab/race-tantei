...
function enrichCandidate(candidate, race, returnPer100) {
  return {
    ...candidate,
    returnPer100,
    oddsBucket: bucket(candidate.assumedOdds, [3, 5, 8, 12, 20, 40, 80, 150, 500], ["o1", "o2", "o3", "o4", "o5", "o6", "o7", "o8", "o9", "o10"]),
    hitBucket: bucket(candidate.hitProbability, [0.005, 0.01, 0.02, 0.04, 0.07, 0.1, 0.15, 0.25, 0.4], ["h1", "h2", "h3", "h4", "h5", "h6", "h7", "h8", "h9", "h10"]),
    rankBucket: bucket(candidate.rankSum, [3, 5, 8, 12], ["r1", "r2", "r3", "r4", "r5"]),
...
function featureKeys(row) {
  ...
  return [
    ["global", "all", 800, 0.05],
    ["type", type, 300, 0.14],
    ["typeOdds", `${type}|${row.oddsBucket}`, 100, 0.18],
    ["typeHit", `${type}|${row.hitBucket}`, 100, 0.15],
    ["typeRank", `${type}|${row.rankBucket}`, 80, 0.1],
    ["typeFirst", `${type}|${row.includesFirst}`, 100, 0.08],
    ["typeField", `${type}|${row.fieldBucket}`, 80, 0.06],
    ["typeTop", `${type}|${row.topBucket}`, 80, 0.06],
    ["typeGap", `${type}|${row.gapBucket}`, 80, 0.06],
    ["typeVenue", `${type}|${row.venue}`, 100, 0.05],
    ["typeRaceNo", `${type}|${row.raceNoBucket}`, 100, 0.05],
    ["cell", `${type}|${row.oddsBucket}|${row.hitBucket}|${row.rankBucket}|${row.includesFirst}`, 35, 0.07]
  ];
}
...
function trainEdgeModel(rows) {
  ...
  const globalHit = global.count > 0 ? global.hits / global.count : 0.1;
  ...
  function estimate(family, key, priorCount) {
    ...
    if (!stat?.count) return { roi: parentReturn, hit: parentHit, count: 0 };
    return {
  ...
  return {
    ...
    score(row) {
      ...
      for (const [family, key, prior, factorWeight] of featureKeys(row)) {
        ...
        roi += Math.max(10, Math.min(350, value.roi)) * factorWeight;
      ...
      row.edgeScore = row.learnedRoi * Math.pow(Math.max(0.001, row.learnedHit), 0.2);
      return row;
    }
  };
}
...
function raceRank(row, mode) {
  ...
  const bestRoi = best?.learnedRoi ?? 0;
  const bestEdge = best?.edgeScore ?? 0;
  if (mode === "edge") return bestEdge;
  if (mode === "roi") return bestRoi;
  if (mode === "confidence") return row.topWinProbability * 100 + row.probabilityGap * 150;
  return bestEdge + row.modelScore * 4 + row.topWinProbability * 25;
}
...
function filteredCandidates(race, policy) {
  ...
  const pool = eligible.length ? eligible : race.candidates.filter((row) => allowed.has(row.betType));
  const fallbackPool = pool.length ? pool : race.candidates;
  return [...fallbackPool].sort((a, b) => b.edgeScore - a.edgeScore || b.learnedHit - a.learnedHit);
}
...
function evaluate(records, policy, target) {
  ...
  return {
    ...
    objective: roi * 0.45 + minMonth * 0.35 + hit * 0.2
  };
}
...
function searchPolicy(records, target, course) {
  ...
  const dimensions = [
    ["minLearnedRoi", [0, 60, 75, 85, 95, 105, 120, 140, 170]],
    ["minLearnedHit", [0, 0.01, 0.02, 0.04, 0.07, 0.1, 0.15, 0.25]],
...
async function main() {
  ...
  for (const race of races) {
    ...
    const candidates = universe.buildBettingCandidateUniverse(predictions, 5).map((candidate) => {
  ...
  const model = trainEdgeModel(trainingCandidates);
  for (const record of records) {
    record.candidates = record.candidates
      .map((candidate) => model.score(candidate))
      .sort((a, b) => b.edgeScore - a.edgeScore || b.learnedHit - a.learnedHit);
  }
  ...
  for (const mode of ["edge", "roi", "confidence", "combined"]) {
    for (const count of [5, 6, 7, 8, 9, 10, 11, 12]) {
      const selected = selectRaces(validation, count, mode);
      const result = evaluate(selected, defaultPolicy, 100);
      raceConfigs.push({ mode, count, result });
    }
  }
  ...
  for (const config of finalists) {
    console.log(`Searching realized-edge policy for ${config.mode}/${config.count}R...`);
    ...
    for (const course of courses) {
      ...
      if (!winner) throw new Error(`NO_REALIZED_EDGE_POLICY:${course}`);
    ...
    searched.push({
      ...config,
      policies,
      aggregate: minimumRoi * 0.45 + minimumMonth * 0.35 + minimumHit * 0.2
    });
  }

  searched.sort((a, b) => b.aggregate - a.aggregate);
  const winner = searched[0];
  if (!winner) throw new Error("NO_REALIZED_EDGE_WINNER");
  ...
  const report = {
    ...
    method: "Train smoothed realized-return ticket signals on 2025-05 through 2026-04. Select race and ticket policy on validation only. Evaluate July holdout once after selection.",
    training: {
    ...
    raceConfigurationFinalists: finalists.map((row) => ({
      mode: row.mode,
      count: row.count,
  ...
  console.log("Realized-edge result: realized-ticket-edge.json");
}
