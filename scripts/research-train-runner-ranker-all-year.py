...
def main():
    ...
    meta={'purpose':'research_only_all_race_year_walkforward_runner_ranker','year':year,'trainEndExclusive':train_end,'evaluationStart':eval_start,'evaluationEnd':eval_end,'trainingRaces':int(train['raceId'].nunique()),'trainingRunners':len(train),'predictionRaces':int(ev['raceId'].nunique()),'predictionRunners':len(ev),'metrics':met,'allEvaluationRacesPredicted':True,'futureRowsUsedForTraining':False,'sameDayRowsUsedForTraining':False,'targetRaceResultUsedForPrediction':False,'syntheticDataUsed':False,'productionDatabaseWritten':False,'productionModelChanged':False}
