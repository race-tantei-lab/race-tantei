...
if (!DEMAND || !META_DIR || !CORPUS || !OUT || !META) throw new Error("--demand, --meta-dir, --corpus, --out and --meta are required");
...
function cnameFromUrl(url){try{return decodeURIComponent(new URL(url).searchParams.get("CNAME")??"");}catch{return "";}}
...
async function findMetaFiles(dir){
  ...
  async function walk(current){for(const entry of await readdir(current,{withFileTypes:true})){const full=path.join(current,entry.name);if(entry.isDirectory())await walk(full);else if(/research-demanded-odds-\d{4}-meta\.json$/.test(entry.name))out.push(full);}}
...
async function main(){
  ...
  for(const file of metaFiles){const meta=JSON.parse(await readFile(file,"utf8"));for(const failure of meta.failures??[]){const rid=String(failure.raceId??"");if(rid)failures.set(rid,failure);}}
  const monthCache=new Map();
  async function monthUrls(month){if(!monthCache.has(month))monthCache.set(month,getArchiveResultUrls(month));return monthCache.get(month);}
  const resolved=[],unresolved=[]; let archiveResolved=0,currentDirectResolved=0,currentArchiveSkipped=0;
  for(const rid of [...failures.keys()].sort()){
    try{
      ...
      if(month==="202608"){
        ...
        currentDirectResolved+=1; currentArchiveSkipped+=1;
      }else{
        try{
          const urls=await monthUrls(month); const prefix=expectedArchivePrefix(race);
      ...
      resolved.push({...demand,resultUrl:resolvedUrl,entryUrl,resultUrlResolutionMethod:resolutionMethod,originalResultUrl:demand.resultUrl??null,syntheticOddsUsed:false,productionDatabaseWritten:false,productionModelChanged:false});
    }catch(error){unresolved.push({raceId:rid,error:error instanceof Error?`${error.name}:${error.message}`:String(error)});}
  }
  ...
  const meta={purpose:"research_only_resolve_failed_odds_urls_via_jra_official_sources",initialFailureCount:failures.size,resolvedCount:resolved.length,archiveResolvedCount:archiveResolved,currentDirectResolvedCount:currentDirectResolved,currentArchiveSkippedCount:currentArchiveSkipped,unresolvedCount:unresolved.length,unresolved,archiveMonthsAttempted:[...monthCache.keys()].sort(),syntheticOddsUsed:false,productionDatabaseWritten:false,productionModelChanged:false};
