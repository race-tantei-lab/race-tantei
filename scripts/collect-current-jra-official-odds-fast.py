...
def parse_win(page: str) -> tuple[list[int], list[tuple[str, float, float]]]:
    ...
    for i, row in enumerate(rows):
        if "馬番" in row and any(cell == "単勝" or cell.startswith("単勝") for cell in row):
            header_index=i; horse_index=row.index("馬番"); win_index=next(j for j,cell in enumerate(row) if cell=="単勝" or cell.startswith("単勝")); break
    if header_index < 0: raise RuntimeError("WIN_HEADER_NOT_FOUND")
    by_horse: dict[int, tuple[float,float]] = {}
    for row in rows[header_index+1:]:
        if len(row) <= max(horse_index,win_index) or not re.fullmatch(r"\d{1,2}",row[horse_index].strip()): continue
        odds=exact_decimal_odds(row[win_index]); horse=int(row[horse_index])
        if odds and 1<=horse<=30: by_horse.setdefault(horse,odds)
    horses=sorted(by_horse)
    if len(horses)<2: raise RuntimeError(f"WIN_HORSES_TOO_FEW:{len(horses)}")
...
def find_type_cnames(page: str, race_date_digits: str, race_no: int) -> dict[str,str]:
    ...
    for bet_type,prefix in TYPE_PREFIX.items():
        ...
        if candidates: result[bet_type]=candidates[0]
    return result
...
def main() -> None:
    ...
    races=[]
    for race in source:
        if race["raceId"] not in AUG9_SELECTED: continue
        seconds=base.seconds_to_start(race.get("startTimeUtc"),captured)
        if seconds is None or seconds <= 15*60: continue
        races.append(race)
    ...
    for race in races:
        ...
        if not entry_url: errors.append(f"RACE:{race['raceId']}:ENTRY_URL_MISSING"); continue
        ...
        try:
            ...
            if not win_seeds: raise RuntimeError("WIN_CNAME_NOT_FOUND")
            win_cname=win_seeds[0]; win_page=runtime.fetch_url(base.JRA_ODDS_URL,cname=win_cname,referer=entry_url); odds_pages+=1
            horses,win_rows=parse_win(win_page); add_records(records,race,"単勝",win_cname,win_page,win_rows,captured_iso,captured); parsed_by_type["単勝"]+=len(win_rows)
            ...
            for bet_type in ["馬連","ワイド","馬単","3連複","3連単"]:
                ...
                if not cname: raise RuntimeError(f"{bet_type}_CNAME_NOT_FOUND")
                ...
                if identity!=(race["raceDate"],race["venue"],race_no): raise RuntimeError(f"{bet_type}_IDENTITY_MISMATCH:{identity}")
                rows=runtime.parse_odds_rows(page,bet_type)
                if not rows: raise RuntimeError(f"{bet_type}_NO_PARSED_ROWS")
                add_records(records,race,bet_type,cname,page,rows,captured_iso,captured); parsed_by_type[bet_type]+=len(rows)
                time.sleep(max(base.REQUEST_PAUSE_SECONDS,0.5))
        except Exception as error:
            errors.append(f"RACE:{race['raceId']}:{type(error).__name__}:{error}")
    rows=list(records.values())
    with gzip.open(ROWS_PATH,"wt",encoding="utf-8") as fh: json.dump(rows,fh,ensure_ascii=False,separators=(",",":"))
    # Persist current single-win odds for runner display. Full exotic matrices are handed directly to inference in this job.
    runtime.insert_rows([row for row in rows if row["betType"]=="単勝"])
    counts=defaultdict(int); covered=defaultdict(set)
    for row in rows: counts[row["betType"]]+=1; covered[row["betType"]].add(row["raceId"])
    report={"generatedAt":captured_iso,"status":"parsed_selected_official_odds" if rows else "waiting_for_official_odds","oddsSource":"jra_official","eligibleFixedTargetCount":len(races),"entryPagesFetched":entry_pages,"oddsPagesFetched":odds_pages,"parsedOddsRows":len(rows),"rowsByBetType":dict(sorted(counts.items())),"racesByBetType":{k:len(v) for k,v in sorted(covered.items())},"errorCount":len(errors),"errors":errors[:100],"rowsFile":ROWS_PATH.name}
