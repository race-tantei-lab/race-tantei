...
def fast_build_tickets(base_mod, demand_mod, pmod, gen, state, bundle, odds_row, rules_by_bet):
    ...
    betting_horses = {
        ...
        if str(horse).isdigit() and (base_mod.midpoint(value) or 0) > 1
    }
    # Historical runner_status can lag a same-day scratch/exclusion. The final
    # official win market is authoritative for which horses were actually
    # betting-eligible at the final-odds snapshot.
    runners = [
        row for row in base_mod.active_runners(bundle)
        if int(row["horseNo"]) in betting_horses
    ]
    hnos = [int(row["horseNo"]) for row in runners]
    n = len(hnos)
    if n < 3:
        raise RuntimeError(f"RUNNERS_TOO_FEW:{rid}:{n}")
    if set(hnos) != betting_horses:
        missing_runner_rows = sorted(betting_horses - set(hnos))
        raise RuntimeError(f"OFFICIAL_WIN_RUNNER_ROW_MISSING:{rid}:{missing_runner_rows}")
    ...
    for horse in hnos:
        ...
        if odd is None or odd <= 1:
            raise RuntimeError(f"WIN_ODDS_INCOMPLETE_AFTER_FILTER:{rid}:{horse}")
        win.append(odd)
