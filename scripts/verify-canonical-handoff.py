...
def run_static(script: str) -> None:
...
def main() -> None:
    ...
    model = load_json("config/ten-year-completed-model.json")
    audit = load_json("analysis-results/ten-year-model-completion-20260812.json")
    state = load_json("models/ten-year-production-state-manifest.json")
    runners = load_json("data/ten-year-runners/manifest.json")
    public = load_json("wrangler.jsonc")
    ...
    if manifest.get("status") != "production" or int(manifest.get("handoffVersion", 0)) < 5:
        fail("canonical manifest production identity mismatch")
    if manifest.get("sourceOfTruth") != "HANDOFF.md":
        fail("canonical sourceOfTruth mismatch")
    if model.get("status") != "completed" or model.get("name") != "ten-year-completed-model":
        fail("completed model identity mismatch")
    if model.get("productionChanged") is not True:
        fail("completed model productionChanged flag mismatch")

    actual_model_sha = sha256_file(manifest["model"]["weights"])
    if actual_model_sha != MODEL_SHA or manifest["model"]["weightsSha256"] != MODEL_SHA:
        fail("canonical model SHA changed")

    if audit.get("completed") is not True or audit.get("allCompletionGatesPassed") is not True:
        fail("completion audit gates not passed")
    if int(audit.get("archive", {}).get("universeRaces", -1)) != 34566:
        fail("canonical universe race count changed")
    if int(audit.get("archive", {}).get("selectedRaces", -1)) != 14410:
        fail("canonical selected race count changed")
    race_audit = audit.get("raceSelectionAudit", {})
    if race_audit.get("targetDayResultsUsedForSelection") is not False:
        fail("target-day selection leakage flag changed")
    if race_audit.get("historicalFinalOddsUsedForSelection") is not False:
        fail("historical final-odds selection flag changed")
    if race_audit.get("syntheticOddsUsed") is not False:
        fail("synthetic-odds audit flag changed")

    if state.get("throughDate") != manifest["model"]["stateThroughDate"]:
    ...
    for path in (manifest["model"]["runnerFeatureState"], manifest["model"]["raceSelectionState"]):
        info = state.get("files", {}).get(path)
        if not isinstance(info, dict) or sha256_file(path) != str(info.get("sha256", "")):
            fail(f"production state hash mismatch: {path}")
    if int(runners.get("races", -1)) != 34566 or int(runners.get("runners", -1)) != 480441:
        fail("runner archive identity mismatch")

    site = manifest["site"]
    if public.get("main") != site.get("entry"):
        fail(f"wrangler main mismatch: {public.get('main')} != {site.get('entry')}")
    if public.get("vars", {}).get("DEPLOY_REVISION") != site.get("revision"):
        fail("public deploy revision mismatch")
    if public.get("vars", {}).get("MODEL_VERSION") != manifest["model"]["name"]:
        fail("public model version mismatch")
    ...
    if len(d1) != 1 or d1[0].get("database_name") != site["d1DatabaseName"] or d1[0].get("database_id") != site["d1DatabaseId"]:
        fail("public D1 identity mismatch")

    prod = manifest["production"]
    expected = {
        "liveDeadlineEntry": "src/live-deadline-entry-v3.ts",
        "liveDeadlineDriverEntry": "src/live-deadline-entry-v2.ts",
    ...
    for key, value in expected.items():
        if prod.get(key) != value:
            fail(f"production architecture mismatch {key}: {prod.get(key)!r} != {value!r}")

    if primary.get("name") != "race-tantei-live-deadline" or primary.get("main") != prod["liveDeadlineEntry"]:
        fail("primary live Worker identity mismatch")
    ...
    if backup.get("vars", {}).get("LIVE_DEADLINE_ROLE") != "backup":
        fail("backup role mismatch")
    for cfg in (primary, backup):
        binding = cfg.get("d1_databases", [])
        if len(binding) != 1 or binding[0].get("database_id") != site["d1DatabaseId"]:
            fail("live Worker D1 binding mismatch")
    ...
    for marker in (
        ...
        require(migration, marker, "D1 guard migration")
