...
assert.equal(DEADLINE_GUARD_ARM_MS, 20 * 60 * 1000, "arm five minutes early so both schedulers make the final immutable by T-15");
assert.equal(shouldDeadlineGuardLock(DEADLINE_GUARD_ARM_MS), true, "the T-20 backup tick must be allowed to arm the final");
assert.equal(shouldDeadlineGuardLock(15 * 60 * 1000 + 1), true, "scheduler seconds before T-15 must remain protected");
