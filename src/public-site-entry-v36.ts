...
const UI_VERSION = "ten-year-completed-public-v36-daily-performance-20260825";
const PERFORMANCE_API_VERSION = "daily-performance-v1-20260825";
...
async function dailyPerformanceResponse(db: D1Database, requestedDate: string): Promise<Response> {
  const today = jstDate();
  ...
  try {
    ...
    return Response.json({
      ok: true,
      version: PERFORMANCE_API_VERSION,
      today,
      summary: summarizeDay(date, selectedResult.results ?? []),
      history,
    }, {
      headers: {
        "cache-control": "no-store, max-age=0",
        "x-race-performance-api": PERFORMANCE_API_VERSION,
      },
    });
  } catch (error) {
...
function performanceStyles(): string {
  return `<style>
...
function performanceBlock(): string {
  return `<section class="daily-performance-wrap" data-daily-performance>
    ...
    <details class="daily-history" open><summary><b>日別回収率</b><span>直近30開催日・公開運用</span></summary><div id="daily-performance-history" class="daily-history-list"><div class="daily-summary-loading">日別成績を読み込み中…</div></div></details>
  </section>`;
}

function performanceScript(today: string): string {
  return `<script>(()=>{
    const TODAY=${JSON.stringify(today)};
    const current=document.getElementById('daily-performance-current');
    const history=document.getElementById('daily-performance-history');
    const title=document.getElementById('daily-performance-title');
    const selectedHeading=document.getElementById('selected-date');
    ...
    const selectedDate=()=>{
      const text=(selectedHeading?.textContent||'').trim();
      if(text.includes('今日'))return TODAY;
      const m=text.match(/(20\\d{2})[\\/-](\\d{2})[\\/-](\\d{2})/);
      return m?m[1]+'-'+m[2]+'-'+m[3]:TODAY;
    };
    const courseRows=(rows)=>rows.map((r)=>{
      const profitClass=r.settledStakeYen>0?tone(r.profitYen):'';
      const profit=r.settledStakeYen>0?signed(r.profitYen):'—';
      return '<div class="daily-course-row"><b>'+esc(r.course)+'</b><span>購入 '+yen(r.settledStakeYen)+'</span><span>払戻 '+yen(r.returnYen)+'</span><strong class="'+profitClass+'">'+profit+'</strong><em>'+pct(r.roiPct)+'</em></div>';
    }).join('');
    const renderCurrent=(s)=>{
      title.textContent=dateLabel(s.date);
      if(!s.finalizedStakeYen){current.innerHTML='<div class="daily-summary-empty">この日の公開買い目はありません。</div>';return;}
      const profitShown=s.settledStakeYen>0?signed(s.profitYen):'—';
      const profitClass=s.settledStakeYen>0?tone(s.profitYen):'';
      const pending=s.pendingStakeYen>0?'未精算 '+yen(s.pendingStakeYen)+'（収支・回収率にはまだ算入していません）':'全て精算済み';
      current.innerHTML='<div class="daily-summary-top">'
        +'<div class="daily-summary-metric"><span>精算済購入</span><b>'+yen(s.settledStakeYen)+'</b></div>'
        +'<div class="daily-summary-metric"><span>払戻</span><b>'+yen(s.returnYen)+'</b></div>'
        +'<div class="daily-summary-metric"><span>収支</span><b class="'+profitClass+'">'+profitShown+'</b></div>'
        +'<div class="daily-summary-metric"><span>回収率</span><b>'+pct(s.roiPct)+'</b></div></div>'
        +'<div class="daily-summary-note">3コース合計（比較用）・'+s.settledRaces+'R精算 / '+s.finalizedRaces+'R確定　'+pending+'　確定済み総購入 '+yen(s.finalizedStakeYen)+'</div>'
        +'<div class="daily-course-list">'+courseRows(s.courses)+'</div>';
    };
    const renderHistory=(days)=>{
      if(!Array.isArray(days)||!days.length){history.innerHTML='<div class="daily-summary-empty">日別の公開運用成績はまだありません。</div>';return;}
      history.innerHTML=days.map((d)=>{
        const profitShown=d.settledStakeYen>0?signed(d.profitYen):'未精算';
        const profitClass=d.settledStakeYen>0?tone(d.profitYen):'';
        return '<details class="daily-history-day"><summary><b>'+shortDate(d.date)+'</b><span>回収率 '+pct(d.roiPct)+'　購入 '+yen(d.settledStakeYen)+'</span><strong class="'+profitClass+'">'+profitShown+'</strong></summary><div class="daily-history-courses">'+courseRows(d.courses)+'</div></details>';
      }).join('');
    };
    const load=async(date,force=false)=>{
      if(!force&&date===lastDate)return;
      lastDate=date;const my=++seq;
      try{
        const res=await fetch('/api/public/daily-performance?date='+encodeURIComponent(date)+'&_rt='+Date.now(),{cache:'no-store',headers:{'cache-control':'no-cache'}});
        if(!res.ok)throw new Error('HTTP_'+res.status);
        const data=await res.json();if(my!==seq||!data?.ok)return;
        renderCurrent(data.summary);renderHistory(data.history);
      }catch(e){if(my!==seq)return;current.innerHTML='<div class="daily-summary-empty">集計を取得できませんでした。</div>';}
    };
    ...
    setInterval(()=>{if(lastDate===TODAY&&document.visibilityState!=='hidden')load(TODAY,true);},15000);
    window.addEventListener('pageshow',()=>{if(lastDate)load(lastDate,true);});
    document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible'&&lastDate)load(lastDate,true);});
  })();</script>`;
}

async function enhanceHome(response: Response, today = jstDate()): Promise<Response> {
  ...
  if (!html.includes("data-daily-performance")) {
    ...
    if (anchor.test(html)) html = html.replace(anchor, `${block}$1`);
    else html = html.replace(/(<div class="section-title"><h2>累計回収率<\/h2>)/, `${block}$1`);
  }
  html = html.replace("</head>", `${performanceStyles()}</head>`);
  html = html.replace("</body>", `${performanceScript(today)}</body>`);
  const headers = new Headers(response.headers);
  headers.delete("content-length");
  headers.set("cache-control", "no-store, max-age=0");
  headers.set("x-race-ui-version", UI_VERSION);
  headers.set("x-race-performance-ui", PERFORMANCE_API_VERSION);
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (path === "/") response = await enhanceHome(response);
    const headers = new Headers(response.headers);
