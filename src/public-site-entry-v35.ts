...
const UI_VERSION = "ten-year-completed-public-v35-t15-start-t10-transparent-20260823";
...
function clarifyTimingLanguage(input: string, path: string): string {
  ...
  const exact: Array<[string, string]> = [
    ...
    ["保存済みの公式オッズ付き予想を確定し、それ以降は再計算・差し替えをしません。", "発走15分前までに最終計算を開始し、通常はそれ以前に確定します。処理中の場合だけ10分前まで反映し、「確定」表示が出た後は再計算・差し替えをしません。"],
    ["買い目確定待ち", "最終計算中（確定前）"],
