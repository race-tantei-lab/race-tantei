...
function courseBudget(course: string): string {
  if (course === "ライト") return "上限2,000円・目安1,600円";
  if (course === "スタンダード") return "上限5,000円・目安4,200円";
  return "上限10,000円・目安8,800円";
}

function metricCard(row: CourseMetric): string {
  return `<section class="course ${row.course}">
    <div class="course-head"><div><span class="label">累計成績</span><h2>${escapeHtml(row.course)}</h2><small>${courseBudget(row.course)}</small></div><strong class="roi ${row.roiPct !== null && row.roiPct >= 100 ? "plus" : "minus"}">${pct(row.roiPct)}</strong></div>
    <div class="stats">
