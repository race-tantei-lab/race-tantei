...
export function renderCoursePerformance(
  ...
  const cards = cumulative.map((row) => `
    <section class="course ${row.course}">
      <div class="course-head"><div><span class="label">${escapeHtml(row.course)}コース</span><h2>${courseBudget(row.course)}</h2></div><strong class="roi ${row.roiPct !== null && row.roiPct >= 100 ? "plus" : "minus"}">${pct(row.roiPct)}</strong></div>
      <div class="stats">
        ...
        <div><small>精算レース</small><b>${row.settledRaces}R</b></div>
      </div>
    </section>`).join("");
