...
function emptyMetric(course: CourseMetric["course"]): CourseMetric {
  return {
    course,
    settledRaces: 0,
    betCount: 0,
...
function mergeHistoricalIntoMonthly(
  ...
  for (const block of historicalMonthly) {
    for (const course of block.courses) {
      const history = validationToMetric(course);
      const index = rows.findIndex((row) => row.month === block.month && row.course === history.course);
      if (index >= 0) {
        const live = rows[index]!;
        rows[index] = { month: block.month, ...combinedMetric(live, history) };
      } else {
        rows.push({ month: block.month, ...history });
      }
    }
  }
...
export function renderCoursePerformance(
  ...
  const courses = cumulative.length > 0
    ? cumulative.map((row) => row.course)
    : (["ライト", "スタンダード", "プレミアム"] as CourseMetric["course"][]);
  const liveMetrics = courses.map((course) => cumulative.find((row) => row.course === course) ?? emptyMetric(course));
  const historicalMetrics = courses.map((course) => validationToMetric(
    historical.find((row) => row.course === course) ?? emptyHistorical(course)
  ));
  const combined = liveMetrics.map((live) => combinedMetric(
    live,
    historicalMetrics.find((row) => row.course === live.course) ?? emptyMetric(live.course)
  ));
  const combinedMonthly = mergeHistoricalIntoMonthly(monthly, historicalMonthly);
  ...
  const period = scope ? `${scope.startDate}〜${scope.endDate}` : "過去レース";
  const progress = scope && !scope.complete ? `（集計中・保存済み${scope.totalRaces}R）` : "";
