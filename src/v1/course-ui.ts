...
function metricCard(row: CourseMetric): string {
  return `<section class="course ${row.course}">
    <div class="course-head"><div><span class="label">総合成績</span><h2>${escapeHtml(row.course)} ${courseBudget(row.course)}</h2></div><strong class="roi ${row.roiPct !== null && row.roiPct >= 100 ? "plus" : "minus"}">${pct(row.roiPct)}</strong></div>
    <div class="stats">
      ...
      <div><small>対象レース</small><b>${row.settledRaces}R</b></div>
    </div>
  </section>`;
}
...
export function renderCoursePerformance(
  ...
  _monthly: Array<CourseMetric & { month: string }>,
