...
interface DashboardRace {
  ...
  startTimeJst: string | null; startTimeUtc: string | null; status: string;
  predictionStatus: string | null; modelVersion: string | null;
  topHorseNo: number | null; topHorseName: string | null;
  winnerHorseNo: number | null; winnerHorseName: string | null;
  betCount: number; lightStake: number; lightReturn: number;
  standardStake: number; standardReturn: number; premiumStake: number; premiumReturn: number;
}

interface CourseMetric { course: BudgetCourse; stake: number; returns: number; races: number; hits: number; }
...
export async function getResultDashboard(db: D1Database, liveModel: string): Promise<{ races: DashboardRace[]; metrics: CourseMetric[] }> {
  const races = await db.prepare(`
    ...
    SELECT r.race_id AS raceId, r.race_date AS raceDate, r.venue, r.race_no AS raceNo,
      ...
      r.status, p.status AS predictionStatus, p.model_version AS modelVersion,
    ...
    FROM rt_races r LEFT JOIN chosen_prediction p ON p.race_id=r.race_id
    WHERE r.race_date >= '${AUG1_DATE}'
    ORDER BY r.race_date, r.venue, r.race_no LIMIT 120
  `).bind(liveModel, liveModel).all<DashboardRace>();

  const metricRows = await db.prepare(`
    WITH ${chosenCte()},
    aug1_done AS (
      SELECT COUNT(DISTINCT race_id) AS count FROM chosen_prediction
    ...
    SELECT CASE
      ...
      WHEN b.bet_type LIKE 'プレミアム｜%' THEN 'プレミアム' END AS course,
    ...
    FROM rt_bets b JOIN chosen_prediction p ON p.id=b.prediction_id
  ...
  return {
    races: races.results.map((r) => ({ ...r, raceNo:Number(r.raceNo), betCount:Number(r.betCount),
      lightStake:Number(r.lightStake), lightReturn:Number(r.lightReturn), standardStake:Number(r.standardStake),
      standardReturn:Number(r.standardReturn), premiumStake:Number(r.premiumStake), premiumReturn:Number(r.premiumReturn) })),
    metrics: COURSES.map(({name}) => ({ course:name, stake:Number(metricMap.get(name)?.stake??0),
      returns:Number(metricMap.get(name)?.returns??0), races:Number(metricMap.get(name)?.races??0), hits:Number(metricMap.get(name)?.hits??0) }))
  };
}

function dateLabel(date:string):string { const m=date.match(/^(\d{4})-(\d{2})-(\d{2})$/); if(!m)return date; const wd=["日","月","火","水","木","金","土"][new Date(Date.UTC(+m[1],+m[2]-1,+m[3])).getUTCDay()]; return `${+m[2]}月${+m[3]}日（${wd}）`; }
...
function courseResult(r:DashboardRace,prefix:string):{stake:number;returns:number}{return{stake:Number(r[`${prefix}Stake` as keyof DashboardRace]??0),returns:Number(r[`${prefix}Return` as keyof DashboardRace]??0)}}

function statusHtml(r:DashboardRace):string {
  if(r.status==="finished"){
    if(r.betCount===0)return `<div class="badge neutral">集計待ち</div><small>結果取得・3コース検証を処理中</small>`;
    const retrospective=r.modelVersion===AUG1_MODEL||r.modelVersion===AUG2_MODEL;
    const lines=COURSES.map(c=>{const x=courseResult(r,c.prefix),profit=x.returns-x.stake,hit=x.returns>0;return `<div class="course-line ${hit?"hit":"miss"}"><b>${c.name}</b><span>${hit?"的中":"不的中"}</span><strong>${formatYen(x.stake)} → ${formatYen(x.returns)}</strong><em>${money(profit)}</em></div>`}).join("");
    return `${retrospective?'<div class="badge neutral">遡及検証</div>':""}<div class="winner">1着 ${r.winnerHorseNo??"—"} ${escapeHtml(r.winnerHorseName??"")}</div>${lines}`;
  }
