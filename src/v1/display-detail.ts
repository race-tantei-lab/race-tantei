...
async function selectDisplayPrediction(
  ...
  if (raceDate === BACKTEST_DATE) {
    ...
    `).bind(raceId, BACKTEST_MODEL).first<PredictionRow>();
  }

  return await db.prepare(`
    SELECT p.id, p.status, p.model_version AS modelVersion,
      p.generated_at AS generatedAt, p.locked_at AS lockedAt
...
export async function getDisplayRaceDetail(db: D1Database, raceId: string): Promise<RaceDetail | null> {
  ...
  const predicted = await db.prepare(`
    SELECT horse_no AS horseNo, horse_name AS horseName, predicted_order AS predictedOrder,
      ...
      expected_value_pct AS expectedValuePct, explanation
  ...
  const bets = await db.prepare(`
    SELECT bet_type AS betType, combination, stake_yen AS stakeYen,
      assumed_odds AS assumedOdds, expected_value_pct AS expectedValuePct,
      settlement_status AS settlementStatus, return_yen AS returnYen
    FROM rt_bets
    WHERE prediction_id=?
      AND (
        bet_type LIKE 'ライト｜%'
        OR bet_type LIKE 'スタンダード｜%'
        OR bet_type LIKE 'プレミアム｜%'
      )
    ORDER BY CASE
      WHEN bet_type LIKE '%｜単勝' THEN 1
      WHEN bet_type LIKE '%｜複勝' THEN 2
      WHEN bet_type LIKE '%｜ワイド' THEN 3
      WHEN bet_type LIKE '%｜馬連' THEN 4
      WHEN bet_type LIKE '%｜馬単' THEN 5
      WHEN bet_type LIKE '%｜3連複' THEN 6
      WHEN bet_type LIKE '%｜3連単' THEN 7
      ELSE 99 END,
      combination
