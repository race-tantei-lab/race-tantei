...
function trainingProgressRows(state: WorkerCalibrationPanelState): string {
  ...
  const scoredTarget = Math.max(0, modelTarget);
  const applyTarget = training.targetRaces;
  ...
  return `<div style="margin-top:13px;padding:12px;border:1px solid #29463f;border-radius:13px;background:#0b1715">
    ...
    <p style="margin:10px 0 0;font-size:11px;opacity:.72">保存済み ${history.storedRaces.toLocaleString("ja-JP")}R。履歴計算用データを含むため、保存数と学習対象数は一致しません。</p>
  </div>`;
}
