...
import { decodeHtmlEntities, jraActionLinks, jraPageText, parseJraOfficialOddsRows, stripJraTags, type JraOddsIdentity } from "./jra-official-odds";
...
export function currentJraRaceNoFromCname(cname: string): number | null {
  ...
  for (let i = values.length - 1; i >= 0; i -= 1) if (values[i] >= 1 && values[i] <= 12) return values[i];
  return null;
}

export function sameFastJraRaceLink(cname: string, dateDigits: string, raceNo: number): boolean {
  return cname.includes(dateDigits) && currentJraRaceNoFromCname(cname) === raceNo;
}
...
export function parseFastJraOddsIdentity(pageHtml: string, cname: string): JraOddsIdentity | null {
  ...
  if (!date || !venue || raceNo == null) return null;
  return { raceDate: `${date[1]}-${date[2].padStart(2, "0")}-${date[3].padStart(2, "0")}`, venue: venue[1], raceNo };
}

export function findFastJraTypeCnames(pageHtml: string, dateDigits: string, raceNo: number): Partial<Record<CompletedBetType, string>> {
  const links = jraActionLinks(pageHtml).filter((link) => sameFastJraRaceLink(link.cname, dateDigits, raceNo));
  const result: Partial<Record<CompletedBetType, string>> = {};
  for (const betType of Object.keys(JRA_FAST_TYPE_PREFIX) as CompletedBetType[]) {
    const found = links.find((link) => link.cname.startsWith(JRA_FAST_TYPE_PREFIX[betType]));
    if (found) result[betType] = found.cname;
  }
  return result;
}
