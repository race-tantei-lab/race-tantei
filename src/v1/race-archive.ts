...
export async function getRaceArchiveIndex(db: D1Database): Promise<RaceArchiveDateSummary[]> {
  const rows = await db.prepare(`
    WITH processed AS (
      SELECT DISTINCT race_id FROM rt_predictions WHERE status='locked'
    ), ticket_state AS (
      SELECT b.race_id,
        1 AS selected,
      ...
      SELECT r.race_date, r.race_id, r.venue,
        CASE WHEN p.race_id IS NULL THEN 0 ELSE 1 END AS processed,
        COALESCE(t.selected,0) AS selected,
        COALESCE(t.hit,0) AS hit
      FROM rt_races r
      LEFT JOIN processed p ON p.race_id=r.race_id
      LEFT JOIN ticket_state t ON t.race_id=r.race_id
      WHERE r.status='finished'
    )
    SELECT race_date AS raceDate,
      COUNT(*) AS totalRaces,
      COUNT(DISTINCT venue) AS venueCount,
      GROUP_CONCAT(DISTINCT venue) AS venues,
      SUM(processed) AS processedRaces,
      SUM(selected) AS selectedRaces,
      SUM(hit) AS hitRaces
    FROM race_state
    GROUP BY race_date
...
export function renderRaceArchiveIndex(rows: RaceArchiveDateSummary[]): string {
  ...
  const blocks = months.map((month, monthIndex) => {
    ...
    const links = dates.map((row) => `<a class="archive-date" href="/history/${encodeURIComponent(row.raceDate)}"><div><b>${escapeHtml(dateLabel(row.raceDate))}</b><span>${escapeHtml(row.venues.join("・") || `${row.venueCount}会場`)}</span></div><div class="archive-date-stats"><strong>${row.totalRaces}R</strong><span>選出 ${row.selectedRaces}R${row.processedRaces < row.totalRaces ? `・集計 ${row.processedRaces}/${row.totalRaces}` : ""}</span></div></a>`).join("");
