...
export function shouldDeadlineGuardLock(remainingMs: number): boolean {
  return Number.isFinite(remainingMs) && remainingMs > 0 && remainingMs <= DEADLINE_GUARD_MS;
}
...
async function loadWorkerModel(db: D1Database): Promise<CompletedModelRuntime> {
  ...
  if (meta.get("ready") !== "1" || meta.get("modelVersion") !== COMPLETED_MODEL_VERSION || meta.get("sourceSha256") !== COMPLETED_MODEL_SHA256) {
    throw new Error("DEADLINE_GUARD_MODEL_IDENTITY_INVALID");
  }
  ...
  if (!generation || !Number.isInteger(chunkCount) || chunkCount <= 0 || !Number.isInteger(byteLength) || byteLength <= 0) throw new Error("DEADLINE_GUARD_MODEL_META_INVALID");
  ...
  if (rows.length !== chunkCount || rows.some((row, index) => Number(row.seq) !== index)) throw new Error("DEADLINE_GUARD_MODEL_CHUNKS_INVALID");
  const decoded = rows.map((row) => decodeBase64(row.dataB64));
  if (decoded.reduce((sum, row) => sum + row.byteLength, 0) !== byteLength) throw new Error("DEADLINE_GUARD_MODEL_LENGTH_INVALID");
...
async function loadSelection(db: D1Database, date: string): Promise<string[]> {
  ...
  if (parsed.sourceModel !== COMPLETED_MODEL_VERSION || parsed.resultDataUsedForTargetDay !== false || !Array.isArray(parsed.selected) || !parsed.selected.length) {
    throw new Error(`DEADLINE_GUARD_SELECTION_INVALID:${date}`);
  }
...
async function orderSelectedRaceIds(db: D1Database, date: string, ids: string[]): Promise<string[]> {
  const rows = await db.prepare("SELECT race_id AS raceId,start_time_utc AS startTimeUtc FROM rt_races WHERE race_date=?")
    .bind(date).all<RaceStartRow>();
  const startById = new Map((rows.results ?? []).map((row) => [String(row.raceId), Date.parse(String(row.startTimeUtc || ""))]));
  return [...ids].sort((a, b) => {
    const ams = startById.get(a);
    const bms = startById.get(b);
    const av = Number.isFinite(ams) ? Number(ams) : Number.POSITIVE_INFINITY;
    const bv = Number.isFinite(bms) ? Number(bms) : Number.POSITIVE_INFINITY;
...
async function loadRace(db: D1Database, raceId: string): Promise<{ race: RaceRecord; runners: RunnerRecord[] }> {
  ...
  if (runners.length < 3) throw new Error(`DEADLINE_GUARD_RUNNERS_TOO_FEW:${raceId}:${runners.length}`);
  return { race, runners };
}
...
function validOfficialPreview(snapshot: CachedOfficialPreview, raceId: string, now: Date): snapshot is CachedOfficialPreview & { tickets: CompletedTicket[]; courseBets: CompletedCourseBet[]; generatedAt: string; oddsFetchedAt: string; oddsSource: string; oddsSnapshotSha256: string } {
  ...
  if (!Number.isFinite(generatedMs) || !Number.isFinite(oddsMs) || generatedMs > now.getTime() + 5_000 || oddsMs > now.getTime() + 5_000) return false;
  if (now.getTime() - generatedMs > MAX_OFFICIAL_PREVIEW_AGE_MS) return false;
...
async function latestOfficialPreview(db: D1Database, raceId: string, now: Date): Promise<(CachedOfficialPreview & { tickets: CompletedTicket[]; courseBets: CompletedCourseBet[]; generatedAt: string; oddsFetchedAt: string; oddsSource: string; oddsSnapshotSha256: string }) | null> {
  const row = await db.prepare("SELECT state_value AS value FROM rt_system_state WHERE state_key=? LIMIT 1")
  ...
  try {
    ...
    return parsed.snapshots.find((snapshot) => validOfficialPreview(snapshot, raceId, now)) ?? null;
  } catch {
    return null;
  }
}
...
export async function runCompletedWorkerDeadlineGuard(env: Env, now = new Date()): Promise<DeadlineGuardAudit> {
  ...
  for (const raceId of ids) {
    try {
      const existing = await publicRows(env.DB, raceId);
      ...
      const start = await env.DB.prepare("SELECT start_time_utc AS startTimeUtc FROM rt_races WHERE race_id=? LIMIT 1")
        .bind(raceId).first<{ startTimeUtc: string | null }>();
      ...
      if (!shouldDeadlineGuardLock(remaining)) { audit.skippedOutsideWindowRaceIds.push(raceId); continue; }
      audit.dueRaceIds.push(raceId);

      const official = await latestOfficialPreview(env.DB, raceId, now);
      if (official) {
        await commitOfficialPreview(env.DB, raceId, official, now);
