...
async function loadSelection(db: D1Database, date: string): Promise<string[]> {
  ...
  if (
    parsed.sourceModel !== COMPLETED_MODEL_VERSION
    || parsed.resultDataUsedForTargetDay !== false
...
function validOfficialPreview(
  snapshot: CachedOfficialPreview,
  raceId: string,
  now: Date,
  startMs: number,
): snapshot is CachedOfficialPreview & {
  tickets: CompletedTicket[];
  courseBets: CompletedCourseBet[];
  generatedAt: string;
  oddsFetchedAt: string;
  ...
  const hardCutoffMs = startMs - DEADLINE_GUARD_MS;
  if (
    !Number.isFinite(generatedMs)
    || !Number.isFinite(oddsMs)
    || generatedMs > now.getTime() + 5_000
    || oddsMs > now.getTime() + 5_000
    || generatedMs > hardCutoffMs
    || oddsMs > hardCutoffMs
  ...
  for (const course of COURSES) {
    ...
    if (
      rows.length !== 2
      || new Set(rows.map((row) => row.betType)).size !== 2
      || rows.reduce((sum, row) => sum + Number(row.stakeYen), 0) !== stakes[0] + stakes[1]
    ) return false;
...
async function latestOfficialPreview(db: D1Database, raceId: string, now: Date, startMs: number) {
  const row = await db.prepare("SELECT state_value AS value FROM rt_system_state WHERE state_key=? LIMIT 1")
    .bind(`${PREVIEW_PREFIX}${raceId}`)
    .first<{ value: string }>();
  ...
  try {
    ...
    return parsed.snapshots.find((snapshot) => validOfficialPreview(snapshot, raceId, now, startMs)) ?? null;
  } catch {
    return null;
  }
}

async function commitCourseBets(
  db: D1Database,
  raceId: string,
  courseBets: CompletedCourseBet[],
  ...
  if (!shouldDeadlineGuardLock(startMs - now.getTime())) {
    throw new Error(`DEADLINE_GUARD_WRITE_OUTSIDE_T15_WINDOW:${raceId}`);
  }
  const existing = await publicRows(db, raceId);
  if (strictComplete(existing)) return;
  if (existing.some((row) => Number(row.sourcePredictionId) !== -2 || row.settlementStatus !== "pending")) {
    throw new Error(`DEADLINE_GUARD_UNSAFE_PARTIAL:${raceId}`);
  }
  const lockedAt = iso(now);
  const statements: D1PreparedStatement[] = [];
  if (existing.length) {
    statements.push(db.prepare("DELETE FROM rt_public_bets WHERE race_id=? AND source_prediction_id=-2 AND settlement_status='pending'").bind(raceId));
  }
...
export async function ensureCompletedRaceFinalAtDeadline(
  env: Env,
  raceId: string,
  now = new Date(),
): Promise<DeadlineEnsureResult> {
  ...
  const startMs = Date.parse(String(race.startTimeUtc || ""));
  if (!Number.isFinite(startMs)) throw new Error(`DEADLINE_GUARD_START_TIME_INVALID:${raceId}`);
  const remainingMs = startMs - now.getTime();
  if (isDeadlineGuardMissed(remainingMs)) return { status: "deadline_missed", raceId, remainingMs };
